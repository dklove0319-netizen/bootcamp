...
규칙:
- reason 은 참가자와 다르게 본 조각 중 가장 선명한 1~2곳에만 — 카메라 기준 건조한 관찰. 기본은 한 문장, 기록에 사실 재료가 없는 평가어 조각이면 '바꿔 쓰는 법 보여주기' 꼴(2~3문장, 예시+되묻기)로. 맞았다/틀렸다 금지. 같게 본 조각은 빈 문자열.
  ...
  try {
    ...
    const items = tagged.map((f, i) => {
      ...
      if (mirror !== f.label && reasonRaw !== "" && budget > 0) {
        reason = reasonRaw.slice(0, 400); // '바꿔 쓰는 법' 꼴(예시+되묻기)이 들어갈 여유 (2026-07-15)
        budget--;
      }
