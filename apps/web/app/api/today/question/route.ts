...
const FALLBACK = {
  ko: "오늘의 기록에서 가장 오래 남는 문장은 무엇인가요?",
  en: "Which sentence from today's record stays with you the longest?",
};
...
- question: reflection 이 있으면 보정 질문 — "이 문장은 0에서 10 중 몇 점쯤 맞나요. 틀렸다면 어느 부분이 가장 다르게 느껴지나요" 형태. 참가자의 원문 표현 중 더 가까운 말을 고르게 해도 좋아요 — 단 원문에 실제로 있는 표현만. reflection 이 없으면: 과거 기록에서 오늘과 나란히 놓을 문장 하나를 그대로 인용하고(quote_date, quote_src) 해석 아래를 여는 되묻는 질문 하나. "왜" 시작 금지, 답 후보 금지(원문 표현 고르기는 예외), 판단·단정·위로·조언 금지.
- 건조하게. 질문은 반드시 물음표(?)로 끝내세요.
...
export async function POST(req: Request): Promise<Response> {
  ...
  if (todayRows.length > 0 && todayRows[0].question_text !== null && todayRows[0].question_text !== "") {
    return Response.json({ question: ensureQuestionMark(todayRows[0].question_text), quoteDate: null, quoteSrc: null });
  }
