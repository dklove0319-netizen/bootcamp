...
- returns: 감정이나 생각이 올라온 뒤 참가자가 몸·현재로 돌아온 동작의 기록(심호흡, 감각으로 주의 이동, 알아차린 뒤 멈춘 것 등)이 서로 다른 날짜에 2번 이상 있으면 그 원문 조각들 — 각각 해당 날짜 기록의 원문을 한 글자도 바꾸지 말고 그대로. 동작 위에 붙은 해석 말고 동작 문장만. 2일 미만이거나 없으면 빈 배열 — 지어내지 마세요. 칭찬·평가 금지, 조각만.
JSON 만: {"reflection":"","evidence":[{"date":"YYYY-MM-DD","src":""}],"returns":[{"date":"YYYY-MM-DD","src":""}],"quote_date":"","quote_src":"","question":"..."}`;
...
export async function GET(req: Request): Promise<Response> {
  ...
  // 21일째 재설문 게이트 (+ 자기 문답 — instrument 'self')
  const ar = await fetch(
    `${store.url}/rest/v1/assessments?user_id=eq.${secret}&select=phase,instrument,answers,total_score,created_at&order=created_at.asc`,
    { headers: store.headers, cache: "no-store" }
  );
  const assessments = ar.ok
    ? ((await ar.json()) as { phase: string; instrument: string; answers: unknown; total_score: number | null; created_at: string }[])
    : [];
  const day0 = assessments.find((a) => a.phase === "day0" && a.instrument === "who5") ?? null;
  const day21 = assessments.find((a) => a.phase === "day21" && a.instrument === "who5") ?? null;
  if (day21 === null) {
    return Response.json({ needsDay21: true });
  }
  // 시작점의 문장 · 21일째의 문장 (사용자가 쓴 원문 그대로 — 없으면 null, 지어내지 않음)
  const selfAt = (phase: string) => {
    const row = assessments.find((a) => a.phase === phase && a.instrument === "self");
    if (row === undefined || !Array.isArray(row.answers)) return null;
    const texts = (row.answers as unknown[]).filter((s): s is string => typeof s === "string" && s !== "");
    return texts.length === 0 ? null : { date: row.created_at.slice(0, 10), texts };
  };
  const selfWords = { day0: selfAt("day0"), day21: selfAt("day21") };
  ...
  let returnedWays: { date: string; src: string }[] = [];
  let finalQuestion: {
  ...
  try {
    ...
    const parsed = JSON.parse(strippedR.includes("{") ? strippedR.slice(strippedR.indexOf("{"), strippedR.lastIndexOf("}") + 1) : strippedR) as {
      ...
      returns?: { date?: string; src?: string }[];
      quote_date?: string; quote_src?: string; question?: string;
    };
    // 돌아온 방식 — 원문 대조 검증 + 서로 다른 날짜 2일 이상만 (반복의 지도와 같은 기준, 아니면 폐기)
    {
      const cand = (Array.isArray(parsed.returns) ? parsed.returns : [])
        .filter(
        ...
        .slice(0, 4);
      returnedWays = new Set(cand.map((ev) => ev.date)).size >= 2 ? cand : [];
    }
