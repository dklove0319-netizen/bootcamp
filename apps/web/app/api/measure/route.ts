...
작업: 참가자가 오늘의 기록을 쓰고, 그중에서 "사실만" 골라 사실 칸에 다시 적었어요. 당신은 그 사실 칸을 성분 단위로 검사해요. 원기록은 맥락 참고용이에요 — 인용은 사실 칸에서만.
...
- question: 오염이 있으면 가장 선명한 오염 하나를 향한 되묻는 질문 ("~에서 실제로 보인 행동은 무엇이었나요"), 깨끗하면 기록 전체를 향한 판정 없는 되묻는 질문. 딱 하나. "왜"로 시작 금지. 질문 뒤에 답의 후보를 채우지 마세요 — 진공은 참가자가 채워요.
- 기록과 같은 언어로 쓰세요. 한국어면 ~이에요/~해요체. 건조하게, 짧게.

다음 JSON 형식으로만 답하세요 (다른 텍스트 금지):
{"items":[{"src":"사실 칸 원문 조각","label":"fact","reason":""}],"question":"되묻는 질문 하나"}`;
...
export async function POST(req: Request): Promise<Response> {
  ...
  if (text.length > MAX_CHARS || facts.length > MAX_CHARS) {
  ...
  const userMessage = `오늘의 기록(맥락 참고용):\n${text}\n\n사실 칸(검사 대상 — 참가자가 사실만 골라 다시 쓴 것):\n${facts}`;
  ...
  try {
    ...
    return Response.json({ items, factCount, delusionCount, clean, question });
  } catch (e) {
