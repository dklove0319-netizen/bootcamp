...
| journeys | 여정 — 코스 1회 수행 (trial3 무료 3일 체험 · mirror21 21일 코스 · 추후 scalpel35) | 본인 | 250행 |
| consents | 동의 이력 (종류·버전·시각·철회) | 본인 | 수백 행 |
| daily_entries | 하루 기록 — trial3·mirror21 공용, 코스별 채우는 칸 다름 | 본인 | 수천 행 |
| step_events | 단계별 진입·제출 로그 (이탈 측정) | 본인 | 수만 행 |
| assessments | 시작점·21일째 설문 (WHO-5 · 탈중심화 척도) | 본인 | 400행 (100명 × 2시점 × 2종) |
| measure_limits | 익명 맛보기 횟수 제한 (IP 해시 + 날짜, 기록 원문 저장 안 함) | 서버 전용 | IP·날짜당 1행 |
| push_subscriptions | 웹 푸시 수신 정보 (브라우저 구독) | 본인 | 100행 (블럭 8에서 추가) |
...
| granted_at | 시각 | O | 동의한 순간 |
| revoked_at | 시각 | | 철회 시각 (null=유효). **연구용 데이터 추출 = revoked_at IS NULL + 환불 아님 (PRICING_SPEC). 철회해도 코스 이용은 유지** |
...
|---|---|---|---|
| id / user_id | | O | 꼬리표 |
| course | 글자 | O | **'trial3'(무료 3일 미니 체험 — 데이터 수집·전환 다리) · 'mirror21'(유료 21일 거울)** · 추후 'scalpel35'(35일 심화) |
...
| instrument | 글자 | O | 'who5' / 'decentering' — 문항 원문·응답 방식은 검증판 그대로 (수정 금지) / 'self' — 자기 문답 원문 (E-6, day0 1문항·day21 2문항, total_score 없음) |
| answers | jsonb | O | 문항별 응답 |
| total_score | 숫자 | | 합계 (서버 계산) |
| created_at | 시각 | O | |

## 표 상세: measure_limits (익명 맛보기 횟수 제한 — 서버 전용)

| 항목 | 종류 | 필수 | 비고 |
|---|---|---|---|
| ip_hash | 글자 | O | 접속 IP를 해시한 값 (**원문 IP 저장 안 함** · 개인정보처리방침에 IP 사용 명시 필요) |
| day | 날짜 | O | ip_hash와 조합 기본키 |
| count | 숫자 | O | 그날 그 IP의 측정 횟수 (비로그인 1일 1회 판정용) |

- **기록 원문은 저장하지 않는다** (스펙: 비로그인 결과 저장 안 함). 오직 "몇 번 눌렀나"만.
- 로그인 사용자의 무료 3일 체험은 여기가 아니라 journeys(course='trial3') + daily_entries에 저장.
...
| measure_limits | 서버(service_role)만 | 서버만 | 공개 정책 없음 — RLS 켜고 정책 0개(기본 차단) |
...
> ⚠️ **실행용 최종본은 `docs/plan/sql/block1.sql` 이다** (2026-07-05 감사 반영 — consents.revoked_at, measure_limits, journeys.trial3 포함). 아래 블록은 참고용 스냅샷이며 최종본과 다를 수 있으니, 실행은 반드시 block1.sql 로 한다.
